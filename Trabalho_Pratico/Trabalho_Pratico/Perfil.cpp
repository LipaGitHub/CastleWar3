...
Perfil::Perfil(char id_perfil) {
	...
	this->caract = vector<Caracteristica *>(); //Faz com que o vector esteja a zero/null
}

void Perfil::adicionaCaracteristicaNoPerfil(int id) {
	caract.push_back(new Caracteristica(id)); //Guarda caracteristica no respetivo Perfil
}

void Perfil::removeCaracteristicaNoPerfil(int id) {
	for (iter2 = caract.begin(); iter2 != caract.end(); iter2++)
...
//INCOMPLETO
void Perfil::mostraPerfil(char letra) {
